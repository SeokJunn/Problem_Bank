...
<body>
    ...
    $questionNum1 = $_POST['questionNum']; #questionNum1 = 1
    $questionNum = $questionNum1 + 1; #questionNum = 2
        
    $query = " select questionNum, question, view1, view2, view3, view4, view5 from problem where probTypeNum in 
    (select probTypeNum from probType where probType='$probType' and probSubject='$subject' and probDate='$probDate' and questionNum=$questionNum1) ";
    $result = $connect->query($query);
    $rows = mysqli_fetch_assoc($result);
        
    $query2 = " select probTypeNum from probType where probType='$probType' and probSubject='$subject' and probDate='$probDate' ";
    $result2 = $connect -> query($query2);
    $rows2 = mysqli_fetch_assoc($result2);
    $probTypeNum = $rows2['probTypeNum'];
    ...
    if(isset($next)) {
        ...
        $query = " insert into submit (id, probTypeNum, questionNum, subAns, isCorrect)
                    values('$_SESSION[userid]', '$probTypeNum', '$questionNum1', '$subAns', (select if ('$subAns' = (select corAns
                                                                                                        from problem
                                                                                                        where probTypeNum='$probTypeNum' and questionNum='$questionNum1'), 1, 0)))
                    ON DUPLICATE KEY UPDATE questionNum='$questionNum1', subAns='$subAns', isCorrect = (select if ('$subAns' = (select corAns
                                                                                                                                from problem
                                                                                                                                where probTypeNum='$probTypeNum' and questionNum='$questionNum1'), 1, 0)) ";
        $result = $connect->query($query);
        }
    ...
    <form action='./solve.php' method='post'>
        <table border="1" id="rounded-corner">
            ...
            <tr>
                ...
                <label for="id1"><td><?php echo $rows['view1']?></td></label>
            </tr>
            <tr>
                <th>2.<input type='radio' name='subAns' value='2' id="id2"></th>
                <label for="id2"><td><?php echo $rows['view2']?></td></label>
            </tr>
            <tr>
                <th>3.<input type='radio' name='subAns' value='3' id="id3"></th>
                <label for="id3"><td><?php echo $rows['view3']?></td></label>
            </tr>
            <tr>
                <th>4.<input type='radio' name='subAns' value='4' id="id4"></th>
                <label for="id4"><td><?php echo $rows['view4']?></td></label>
            </tr>
            <tr>
                <th>5.<input type='radio' name='subAns' value='5' id="id5"></th>
                <label for="id5"><td><?php echo $probTypeNum?></td></label>
            </tr>
            ...
            <div class="d1" style="float:left;">
                <input type='button' name="next" value='종료' id="button" onclick="location.href='./problem.php'" style="width:150px; margin-left:650px; margin-bottom:20px;">
            </div>

        </table>
        <br>
        <div style="left;">
            <input type='submit' name="next" value='다음' id="button" style="padding-left:48px; padding-right:48px;">
